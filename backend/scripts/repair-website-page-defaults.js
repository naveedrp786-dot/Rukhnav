...
async function run() {
    ...
    try {
        ...
        if (!row) {
            throw new Error(
                "website_settings row id=1 was not found."
            );
        }
        ...
        console.log(
            "Missing Website Management page defaults were added successfully."
        );

        console.log(
            "Existing homepage, branding, theme and contact values were preserved."
        );

        console.log(
            `Page groups now available: ${Object.keys(settings.pages || {}).join(", ")}`
        );
        ...
        console.error(
            "Website Management defaults repair failed:",
            error
        );
