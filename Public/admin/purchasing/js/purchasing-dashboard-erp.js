...
async function requestDashboard() {
    const response = await fetch(
        `${API_BASE_URL}/api/purchasing-dashboard`,
        {
            headers: {
...
function renderSummary(summary) {
    const cards = [
        ...
        ["Open Purchase Orders",
            Number(summary.draft_purchase_orders || 0) +
            Number(summary.approved_purchase_orders || 0) +
            Number(summary.ordered_purchase_orders || 0) +
            Number(summary.partial_receipts || 0),
            "fa-hourglass-half"
        ],
        ...
        ["This Month Purchases", formatMoney(summary.month_purchases), "fa-calendar-days"],
        ...
        ["Return Value", formatMoney(summary.total_return_value), "fa-arrow-rotate-left"],
            ...
            ? rows.map(
                row => `
                    <tr>
                        ...
                        <td>${row.purchased_quantity || 0}</td>
